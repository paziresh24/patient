...
export const EditCenterInfo = ({ onSubmit, onCancel, defaultValues }: EditCenterInfoProps) => {
  ...
  useEffect(() => {
    const latSelcted =
      cities.find(({ id }) => id === dataAddress?.city?.value)?.lat ??
      provinces.find(({ id }) => id === dataAddress?.province?.value)?.lat;
    const lngSelcted =
      cities.find(({ id }) => id === dataAddress?.city?.value)?.lon ??
      provinces.find(({ id }) => id === dataAddress?.province?.value)?.lon;
    ...
    <div className="flex flex-col space-y-4">
      ...
      <div className="h-40 overflow-hidden rounded-lg">
        <Map
          lat={dataAddress.lat}
          lng={dataAddress.lng}
          sendPosition={({ lat, lng }: { lat: number; lng: number }) =>
            setDataAddress((prev) => ({ ...prev, lat, lng }))
          }
          zoom={mapZoom}
        />
      </div>
      <div className="flex space-s-4">
        <Button
          ...
          variant="contained"
          disabled={
            !dataAddress.city?.label || !dataAddress.province?.label || !dataAddress.address
          }

...
export const SelecSymptoms = (props: SelecSymptomsProps) => {
  const { title, placeholder, modalTitle, symptoms, setSelectedSymptoms, selectedSymptoms, setSearchText, searchText } = props;
    ...
    <>
      <div>
        <Text
          fontSize="sm"
          fontWeight="medium"
          className="text-blue-400 cursor-pointer flex items-center gap-2"
          onClick={() => setIsOpenSelectSickness(true)}
        >
          <AddIcon />
          {title}
        </Text>
        ...
        >
          <div className="p-4">
            <div className="flex flex-wrap gap-1">
              {selectedSymptoms.map((symptoms, index) => (
                <>
                  <Chips
                    key={symptoms.id ?? index}
                    handleRemove={() => handleRemoveItems(symptoms.id)}
                    className="h-8 rounded-[5px] mb-4 cursor-pointer"
                  >

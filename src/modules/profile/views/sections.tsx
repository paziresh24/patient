    ...
    // Seo Box
    {
      ...
      function: () => {
        ...
        const getSafeDisplayName = () => {
          ...
          return displayName || 'پزشک';
        };
        
        const safeDisplayName = getSafeDisplayName();
        
        const about = `<p>${safeDisplayName}، ${doctorExpertise ?? 'سایر'} در شهر ${center?.city ?? '(ثبت نشده)'} است. مطب ${
          safeDisplayName
        ...
        <p>تاکنون   ${convertLongToCompactNumber(history?.count_of_page_view) ?? 0} نفر از پروفایل ${safeDisplayName}، ${
        ...
        ${
          center?.freeturn_text
            ? `<p>زودترین زمان رزرو نوبت از مطب ${safeDisplayName} ${center?.freeturn_text} می‌باشد که می‌توانید از طریق وبسایت و یا اپلیکیشن نوبت‌دهی پذیرش۲۴ نوبت خود را به صورت اینترنتی و غیرحضوری دریافت کنید.</p>`
            : ''
        }

...
export const SelectUser = (props: SelectUserProps) => {
  const { onSelect, className, shouldShowMessengers, messengers } = props;
  const userInfo = useUserInfoStore(state => state.info);
  const isLogin = useUserInfoStore(state => state.isLogin);
  const isPendingUser = useUserInfoStore(state => state.pending);
  ...
  useEffect(() => {
    ...
    if (isLogin && userInfo && !isPendingUser) {
      mutate();
  ...
  }, [isLogin, userInfo, isPendingUser]);
  ...
  const getUserWithId = (id: string) => {
    return data?.data?.result?.some((item: any) => item.id === id)
      ? { isSubUser: true, ...data?.data?.result?.find((item: any) => item.id === id) }
      : { isSubUser: false, ...userInfo };
  };

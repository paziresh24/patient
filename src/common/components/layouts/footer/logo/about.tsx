import { memo } from 'react';

const About = () => {
  return (
    <svg className="overflow-hidden" xmlns="http://www.w3.org/2000/svg" width="40" height="40" viewBox="0 0 24.96 23.63">
      <title>icon</title>
      <g>
        <g>
          <g>
            <path
              d="M2.38,4.41H8.09L4.51,19.2s-.73,2.68.72,3.54C1.35,20.92-.59,18.28.16,14.35S2.38,4.41,2.38,4.41Z"
              fill="currentColor"
            ></path>
            <path
              d="M19.64,4.41H25L22.67,14.92s-.9,5-8.89,7.82a5,5,0,0,0,2.38-3.29C16.89,16.9,19.64,4.41,19.64,4.41Z"
              fill="currentColor"
            ></path>
            <path
              d="M15,0S6.55,5.39,6.73,13.88a16.22,16.22,0,0,0,2.9,9.75S2,10.78,16.82,4.7C16.27,2.9,15,0,15,0Z"
              fill="currentColor"
            ></path>
            <path d="M13.78,7,17,5.32s3.77,9.91-6.9,18.22C10.06,23.54,19.37,16.21,13.78,7Z" fill="currentColor"></path>
          </g>
        </g>
      </g>
    </svg>
  );
};

export default memo(About);

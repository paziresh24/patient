...
export const PlasmicSuggestedDoctor__VariantProps = new Array<VariantPropType>();
...
export const PlasmicSuggestedDoctor__ArgProps = new Array<ArgPropType>('categoryTitle', 'categoryValue');
...
function PlasmicSuggestedDoctor__RenderFunc(props: {
  ...
  const styleTokensClassNames_fragment_design_system = useStyleTokens_fragment_design_system();
  ...
  return (
    <ApiRequest
      ...
      onError={async (...eventArgs: any) => {
        generateStateOnChangeProp($state, ['apiRequest', 'error']).apply(null, eventArgs);
      }}
      onLoading={async (...eventArgs: any) => {
        generateStateOnChangeProp($state, ['apiRequest', 'loading']).apply(null, eventArgs);
      }}
      onSuccess={async (...eventArgs: any) => {
        generateStateOnChangeProp($state, ['apiRequest', 'data']).apply(null, eventArgs);
      }}
      ref={ref => {
        $refs['apiRequest'] = ref;
      }}
      url={` ${(() => {
        const category = !!$props.categoryValue ? $props.categoryValue : 'general-practitioner';
    ...
    >
      <div className={classNames(projectcss.all, sty.freeBox__azUh)}>
        ...
        {(_par => (!_par ? [] : Array.isArray(_par) ? _par : [_par]))(
          (() => {
            try {
              return (() => {
                ...
                return [$state.apiRequest.data.search.result.slice(0, 3)?.[random]];
              })();
            } catch (e) {
          ...
          return (
            <div className={classNames(projectcss.all, sty.freeBox__qrbDp)} key={currentIndex}>
              <ProductCard
        ...
        <Button
          ...
          onClick={async event => {
            ...
            $steps['goToHttpsWwwPaziresh24ComConsultFromRecommendSection1'] = !$props.categoryValue
              ? (() => {
                  const actionArgs = {
                    destination: 'https://www.paziresh24.com/consult?from_recommend_section=1',
                  };
                  return (({ destination }) => {
                    if (typeof destination === 'string' && destination.startsWith('#')) {
                      document.getElementById(destination.substr(1)).scrollIntoView({ behavior: 'smooth' });
              ...
              : undefined;
            if (
              $steps['goToHttpsWwwPaziresh24ComConsultFromRecommendSection1'] != null &&
              typeof $steps['goToHttpsWwwPaziresh24ComConsultFromRecommendSection1'] === 'object' &&
              typeof $steps['goToHttpsWwwPaziresh24ComConsultFromRecommendSection1'].then === 'function'
            ) {
              $steps['goToHttpsWwwPaziresh24ComConsultFromRecommendSection1'] = await $steps[
...
type NodeOverridesType<T extends NodeNameType> = Pick<PlasmicSuggestedDoctor__OverridesType, DescendantsType<T>>;
